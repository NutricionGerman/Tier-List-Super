...
export const TierListVisual: React.FC<TierListVisualProps> = ({
  ...
  return (
    <div className="bg-white border border-editorial-border p-4 space-y-4 select-none font-sans">
      ...
      <div className="space-y-3">
        {tiers.map((tier) => {
          ...
          return (
            <div
              key={tier}
              className="flex border border-editorial-border bg-[#FCFAF7] min-h-[100px]"
            >
              {/* Left Column: Tier Identifier */}
              <div
                className={`w-14 shrink-0 flex flex-col items-center justify-center border-r border-editorial-border font-sans font-black text-2xl tracking-tighter shadow-inner ${tierLabelStyles[tier]}`}
              >
                <span>{tier}</span>
                <span className="text-[7px] font-bold tracking-widest uppercase opacity-85 mt-0.5">
                  TIER
                </span>
              </div>

              {/* Right Column: Horizontal Scroll of Cheese Cards */}
              <div className="flex-1 overflow-x-auto no-scrollbar py-2 px-3 flex items-center gap-2.5">
                {tierProducts.length > 0 ? (
                  tierProducts.map((p) => {
                    ...
                    return (
                      <div
                        ...
                        className="w-[105px] shrink-0 bg-white border border-editorial-border hover:border-editorial-text transition-all duration-200 cursor-pointer flex flex-col justify-between h-[115px] group relative"
                      >
                        {/* Image Container */}
                        <div className="relative h-12 w-full overflow-hidden border-b border-editorial-border shrink-0 bg-stone-50">
                          <img
                            src={p.imageUrl}
                            alt={p.name}
                        ...
                        <div className="p-1.5 flex-1 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between pt-1">
                            ...
                            <button
                              ...
                              className={`text-[7px] font-sans font-bold uppercase tracking-widest px-1.5 py-0.5 border transition-all ${
                                isSelected
                  ...
                  <div className="flex-1 flex items-center justify-center py-4">
                    <span className="text-[9px] text-editorial-muted italic font-serif">
                      Sin quesos analizados en este tier
                    </span>
                  </div>
                )}
              </div>
      ...
      <div className="bg-[#F4F1EA] p-2 border border-editorial-border flex items-start gap-1.5">
        <span className="text-[9px] text-editorial-muted leading-relaxed font-serif italic">
